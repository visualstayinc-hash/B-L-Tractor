...
## DIFFERENTIATION SYSTEM (added in a later revision — see PROGRESS.md)

A new "The difference" section (`#difference`, between Work and Process) states why Clearline is different, integrated into the existing page narrative rather than a generic "why us" grid. Before writing any of this, three factual claims were confirmed directly with the user rather than assumed or invented, since they are claims about a real person and a real business that real prospects will read and act on:

- **Age/ambition (D01):** confirmed to be *implied, not stated directly* — the site never says "young." It implies it through "Evan runs Clearline alone" / solo-operator framing, matching the brief's own instruction not to make age the pitch.
- **In-person appointments (D03):** confirmed as a real, current practice — stated as plain fact ("he meets in person, Texas Panhandle wide"), not conditional language.
- **Proactive preview before contact (D04/D05):** confirmed as a real, established practice (not aspirational) — described with conditional language throughout ("may," "sometimes," "occasionally," "depending on the business") per the brief's own requirement, and explicitly disclosed as not guaranteed for every prospect.

No client examples, results, conversion numbers, testimonials, or years-in-business were added — none exist to cite, consistent with the honesty note above. A claim-validation grep pass (`banned absolute claims`, `banned marketing clichés`, fabrication-risk terms) confirmed zero violations before shipping — see QUALITY_GATE.md V20-V23.
